...
local function saveConfig(name, data)
    ...
    if ok then writefile(path, encoded); return true end
    return false
end
...
local function getFont()
    ...
    if writefile and readfile and isfile and getcustomasset then
        ...
        local success, content = pcall(function() return game:HttpGet(fontUrl) end)
        if not success or not content or content == "" then return Font.fromEnum(Enum.Font.Code) end
...
local function CreateObj(ClassName, Params)
    ...
    for i, v in pairs(Params) do Obj[i] = v end
    return Obj
end
...
local TAG_COLORS = {
    red    = "ff4444", green  = "44ff88", yellow = "ffdd44", blue   = "44aaff",
    orange = "ff8833", white  = "ffffff", gray   = "aaaaaa", purple = "aa66ff", cyan = "44ffee",
}

local function parseRichTags(str)
    str = str:gsub("{(%a+)}(.-)({/%1})", function(tag, content)
        local hex = TAG_COLORS[tag:lower()]
        if hex then return ('<font color="#' .. hex .. '">' .. content .. "</font>") end
        return content
    end)
...
local function spawnNotif(rawText, duration)
    ...
    local Wrapper = CreateObj("Frame", {
        Parent = NotifContainer, BackgroundTransparency = 1, BorderSizePixel = 0,
        Size = UDim2.new(1, 0, 0, 0), AutomaticSize = Enum.AutomaticSize.Y,
        ClipsDescendants = false, LayoutOrder = notifCount,
    })
    local Card = CreateObj("Frame", {
        Parent = Wrapper, BackgroundColor3 = Library.Configuration.Background,
        BorderSizePixel = 0, AnchorPoint = Vector2.new(0, 0),
        Position = UDim2.new(0, -300, 0, 0), Size = UDim2.new(1, 0, 0, 0),
        AutomaticSize = Enum.AutomaticSize.Y, ClipsDescendants = false,
    })
    CreateObj("UIStroke", { Parent = Card, Color = Library.Configuration.Accent, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border })
    CreateObj("Frame", { Parent = Card, BackgroundColor3 = Library.Configuration.ActiveToggle, BorderSizePixel = 0, Position = UDim2.new(0,0,0,0), Size = UDim2.new(0,2,1,0) })
    local Inner = CreateObj("Frame", {
        Parent = Card, BackgroundColor3 = Library.Configuration.Inner,
        BorderSizePixel = 0, Position = UDim2.new(0,2,0,0),
        Size = UDim2.new(1,-2,0,0), AutomaticSize = Enum.AutomaticSize.Y, ClipsDescendants = false,
    })
    CreateObj("UIPadding", { Parent = Inner, PaddingLeft = UDim.new(0,8), PaddingRight = UDim.new(0,8), PaddingTop = UDim.new(0,6), PaddingBottom = UDim.new(0,6) })
    CreateObj("TextLabel", {
        Parent = Inner, BackgroundTransparency = 1, Size = UDim2.new(1,0,0,0),
        AutomaticSize = Enum.AutomaticSize.Y, Text = parseRichTags(tostring(rawText)),
        TextColor3 = Color3.fromRGB(200,200,200), TextSize = 12, FontFace = UIFont,
        TextXAlignment = Enum.TextXAlignment.Left, TextYAlignment = Enum.TextYAlignment.Top,
        TextWrapped = true, RichText = true,
    })
    local BarTrack = CreateObj("Frame", {
        Parent = Card, BackgroundColor3 = Color3.fromRGB(10,10,10),
        BorderSizePixel = 0, AnchorPoint = Vector2.new(0,1),
        Position = UDim2.new(0,2,1,0), Size = UDim2.new(1,-2,0,2),
    })
    local BarFill = CreateObj("Frame", { Parent = BarTrack, BackgroundColor3 = Library.Configuration.ActiveToggle, BorderSizePixel = 0, Size = UDim2.new(1,0,1,0) })

    TweenService:Create(Card, TweenInfo.new(ANIM_IN, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Position = UDim2.new(0,0,0,0) }):Play()
    TweenService:Create(BarFill, TweenInfo.new(duration, Enum.EasingStyle.Linear), { Size = UDim2.new(0,0,1,0) }):Play()

    task.delay(duration, function()
        if not Card or not Card.Parent then return end
        local slideOut = TweenService:Create(Card, TweenInfo.new(ANIM_OUT, Enum.EasingStyle.Quad, Enum.EasingDirection.In), { Position = UDim2.new(0,-300,0,0) })
        slideOut:Play()
        slideOut.Completed:Connect(function() if Wrapper and Wrapper.Parent then Wrapper:Destroy() end end)
    end)
end

function Library:Notify(text, duration) spawnNotif(text, duration) end
...
local function applyBackground(color)   Library.Configuration.Background   = color; for _,f in ipairs(ScreenGui:GetDescendants()) do if f:IsA("Frame") and f.Name == "MainFrame" then f.BackgroundColor3 = color end; if f:IsA("Frame") and (f.Name == "SideFrame" or f.Name == "SideFrame2") then f.BackgroundColor3 = color end end end
local function applyInner(color)        Library.Configuration.Inner        = color; for _,f in ipairs(ScreenGui:GetDescendants()) do if f:IsA("Frame") and f.Name == "InnerFrame" then f.BackgroundColor3 = color end; if f:IsA("Frame") and (f.Name == "SideInnerFrame" or f.Name == "SideInnerFrame2") then f.BackgroundColor3 = color end end end
local function applyTopBar(color)       Library.Configuration.TopBar       = color; for _,f in ipairs(ScreenGui:GetDescendants()) do if f:IsA("Frame") and f.Name == "TopBar" then f.BackgroundColor3 = color end end end
local function applyAccent(color)       Library.Configuration.Accent       = color; for _,s in ipairs(ScreenGui:GetDescendants()) do if s:IsA("UIStroke") then s.Color = color end; if s:IsA("ScrollingFrame") then s.ScrollBarImageColor3 = color end end end
local function applyTabActive(color)    Library.Configuration.TabActive    = color end
local function applyTabInactive(color)  Library.Configuration.TabInactive  = color end
local function applyActiveToggle(color) Library.Configuration.ActiveToggle = color; for _,f in ipairs(ScreenGui:GetDescendants()) do if f:IsA("Frame") and f.Name == "Fill" then f.BackgroundColor3 = color end; if f:IsA("Frame") and f.Name == "SliderFill" then f.BackgroundColor3 = color end end end
...
function Library:CreateWindow(Params)
    ...
    local MainFrame = CreateObj("Frame", {
        Name = "MainFrame", Parent = ScreenGui, BackgroundColor3 = Library.Configuration.Background,
        BorderSizePixel = 0, AnchorPoint = Vector2.new(0.5,0.5),
        Position = UDim2.new(0.5,0,0.5,0), Size = UDim2.new(0,600,0,650)
    })
    CreateObj("UIStroke", { Parent = MainFrame, Color = Library.Configuration.Accent, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border })

    local InnerFrame = CreateObj("Frame", {
        Name = "InnerFrame", Parent = MainFrame, BackgroundColor3 = Library.Configuration.Inner,
        BorderSizePixel = 0, AnchorPoint = Vector2.new(0.5,0.5),
        Position = UDim2.new(0.5,0,0.5,0), Size = UDim2.new(1,-50,1,-50)
    })
    CreateObj("UIStroke", { Parent = InnerFrame, Color = Library.Configuration.Accent, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border })

    local TopBar = CreateObj("Frame", {
        Name = "TopBar", Parent = InnerFrame, BackgroundColor3 = Library.Configuration.TopBar,
        BorderSizePixel = 0, AnchorPoint = Vector2.new(0.5,0),
        Position = UDim2.new(0.5,0,0,0), Size = UDim2.new(1,0,0,28)
    })
    CreateObj("UIStroke", { Parent = TopBar, Color = Library.Configuration.Accent, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border })
    ...
    trackConn(UserInputService.InputBegan:Connect(function(input, processed)
        ...
        local abs = TopBar.AbsolutePosition; local size = TopBar.AbsoluteSize
        if mouse.X >= abs.X and mouse.X <= abs.X+size.X and mouse.Y >= abs.Y and mouse.Y <= abs.Y+size.Y then
            dragging = true; dragStartMouse = mouse; dragStartPos = MainFrame.Position
        end
    end))
    trackConn(UserInputService.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then dragging = false end end))
    trackConn(UserInputService.InputChanged:Connect(function(input)
        if not dragging then return end
        if input.UserInputType ~= Enum.UserInputType.MouseMovement then return end
        local delta = Vector2.new(input.Position.X, input.Position.Y) - dragStartMouse
        local screenSize = ScreenGui.AbsoluteSize; local frameSize = MainFrame.AbsoluteSize
    ...
    local TabScrollFrame = CreateObj("ScrollingFrame", {
        Parent = TopBar, BackgroundTransparency = 1, BorderSizePixel = 0,
        Position = UDim2.new(0,0,0,0), Size = UDim2.new(1,0,1,0),
        ScrollBarThickness = 0, ScrollingDirection = Enum.ScrollingDirection.X,
        CanvasSize = UDim2.new(0,0,0,0), AutomaticCanvasSize = Enum.AutomaticSize.X, ClipsDescendants = true
    })
    local TabButtonContainer = CreateObj("Frame", {
        Parent = TabScrollFrame, BackgroundTransparency = 1, BorderSizePixel = 0,
        Position = UDim2.new(0,0,0,0), Size = UDim2.new(1,0,1,0), AutomaticSize = Enum.AutomaticSize.X
    })
    CreateObj("UIPadding", { Parent = TabButtonContainer, PaddingLeft = UDim.new(0,4), PaddingRight = UDim.new(0,4), PaddingTop = UDim.new(0,0), PaddingBottom = UDim.new(0,0) })
    CreateObj("UIListLayout", { Parent = TabButtonContainer, FillDirection = Enum.FillDirection.Horizontal, HorizontalAlignment = Enum.HorizontalAlignment.Left, VerticalAlignment = Enum.VerticalAlignment.Center, Padding = UDim.new(0,4), SortOrder = Enum.SortOrder.LayoutOrder })

    local ContentArea = CreateObj("Frame", { Parent = InnerFrame, BackgroundTransparency = 1, BorderSizePixel = 0, Position = UDim2.new(0,0,0,28), Size = UDim2.new(1,0,1,-28) })

    CreateObj("TextLabel", {
        Parent = InnerFrame, BackgroundTransparency = 1, AnchorPoint = Vector2.new(0.5,0),
        Position = UDim2.new(0.5,0,0,-21.5), Size = UDim2.new(1,-10,0,18),
        Text = Params.Title or "Untitled", TextColor3 = Color3.fromRGB(255,255,255),
    ...
    local SideFrame = CreateObj("Frame", {
        Name = "SideFrame", Parent = MainFrame, BackgroundColor3 = Library.Configuration.Background,
        BorderSizePixel = 0, AnchorPoint = Vector2.new(0,0),
        Position = UDim2.new(1,8,0,0), Size = UDim2.new(0,200,0.5,-4)
    })
    CreateObj("UIStroke", { Parent = SideFrame, Color = Library.Configuration.Accent, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border })

    local SideInnerFrame = CreateObj("Frame", {
        Name = "SideInnerFrame", Parent = SideFrame, BackgroundColor3 = Library.Configuration.Inner,
        BorderSizePixel = 0, AnchorPoint = Vector2.new(0.5,1),
        Position = UDim2.new(0.5,0,1,-10), Size = UDim2.new(1,-20,1,-35)
    })
    CreateObj("UIStroke", { Parent = SideInnerFrame, Color = Library.Configuration.Accent, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border })
    CreateObj("TextLabel", { Parent = SideInnerFrame, BackgroundTransparency = 1, AnchorPoint = Vector2.new(0.5,0), Position = UDim2.new(0.5,0,0,-21.5), Size = UDim2.new(1,-10,0,18), Text = "Changelogs", TextColor3 = Color3.fromRGB(255,255,255), TextSize = 12, FontFace = UIFont, TextXAlignment = Enum.TextXAlignment.Center, TextYAlignment = Enum.TextYAlignment.Center, RichText = false })

    local Scroll = CreateObj("ScrollingFrame", { Parent = SideInnerFrame, BackgroundTransparency = 1, BorderSizePixel = 0, Position = UDim2.new(0,5,0,5), Size = UDim2.new(1,-10,1,-10), ScrollBarThickness = 2, ScrollBarImageColor3 = Library.Configuration.Accent, CanvasSize = UDim2.new(0,0,0,0), AutomaticCanvasSize = Enum.AutomaticSize.Y })
    CreateObj("UIListLayout", { Parent = Scroll, FillDirection = Enum.FillDirection.Vertical, HorizontalAlignment = Enum.HorizontalAlignment.Left, VerticalAlignment = Enum.VerticalAlignment.Top, Padding = UDim.new(0,3), SortOrder = Enum.SortOrder.LayoutOrder })

    local SideFrame2 = CreateObj("Frame", {
        Name = "SideFrame2", Parent = MainFrame, BackgroundColor3 = Library.Configuration.Background,
        BorderSizePixel = 0, AnchorPoint = Vector2.new(0,0),
        Position = UDim2.new(1,8,0.5,4), Size = UDim2.new(0,200,0.5,-4)
    })
    CreateObj("UIStroke", { Parent = SideFrame2, Color = Library.Configuration.Accent, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border })

    local SideInnerFrame2 = CreateObj("Frame", {
        Name = "SideInnerFrame2", Parent = SideFrame2, BackgroundColor3 = Library.Configuration.Inner,
        BorderSizePixel = 0, AnchorPoint = Vector2.new(0.5,1),
        Position = UDim2.new(0.5,0,1,-10), Size = UDim2.new(1,-20,1,-35)
    })
    CreateObj("UIStroke", { Parent = SideInnerFrame2, Color = Library.Configuration.Accent, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border })
    CreateObj("TextLabel", { Parent = SideInnerFrame2, BackgroundTransparency = 1, AnchorPoint = Vector2.new(0.5,0), Position = UDim2.new(0.5,0,0,-21.5), Size = UDim2.new(1,-10,0,18), Text = "kill yourself <3", TextColor3 = Color3.fromRGB(255,255,255), TextSize = 12, FontFace = UIFont, TextXAlignment = Enum.TextXAlignment.Center, TextYAlignment = Enum.TextYAlignment.Center, RichText = false })
    CreateObj("ImageLabel", { Parent = SideInnerFrame2, BackgroundTransparency = 1, AnchorPoint = Vector2.new(0.5,0.5), Position = UDim2.new(0.5,0,0.5,0), Size = UDim2.new(1,0,1,0), Image = Image, ImageColor3 = Color3.fromRGB(255,255,255), ImageTransparency = 0, ZIndex = 2 })

    local Scroll2 = CreateObj("ScrollingFrame", { Parent = SideInnerFrame2, BackgroundTransparency = 1, BorderSizePixel = 0, Position = UDim2.new(0,5,0,5), Size = UDim2.new(1,-10,1,-10), ScrollBarThickness = 2, ScrollBarImageColor3 = Library.Configuration.Accent, CanvasSize = UDim2.new(0,0,0,0), AutomaticCanvasSize = Enum.AutomaticSize.Y })
    CreateObj("UIListLayout", { Parent = Scroll2, FillDirection = Enum.FillDirection.Vertical, HorizontalAlignment = Enum.HorizontalAlignment.Left, VerticalAlignment = Enum.VerticalAlignment.Top, Padding = UDim.new(0,3), SortOrder = Enum.SortOrder.LayoutOrder })

    for i = 1, #Clogs do
        CreateObj("TextLabel", { Parent = Scroll, BackgroundTransparency = 1, Size = UDim2.new(1,0,0,14), Text = Clogs[i], TextColor3 = Color3.fromRGB(160,160,160), TextSize = 12, FontFace = UIFont, TextXAlignment = Enum.TextXAlignment.Left, TextYAlignment = Enum.TextYAlignment.Center, TextWrapped = false, RichText = false, LayoutOrder = i })
    end
    ...
    function Window:Unload()
        for _, conn in ipairs(Library._connections) do pcall(function() conn:Disconnect() end) end
    ...
    function Window:AddTab(name)
        ...
        local TabBtn = CreateObj("TextButton", {
            Parent = TabButtonContainer, BackgroundColor3 = Library.Configuration.TabInactive,
            BorderSizePixel = 0, Size = UDim2.new(0,btnWidth,0,20),
            Text = name, TextColor3 = Library.Configuration.TabTextDim,
            TextSize = 12, FontFace = UIFont, AutoButtonColor = false, LayoutOrder = #tabButtons + 1
        })
        CreateObj("UIStroke", { Parent = TabBtn, Color = Library.Configuration.Accent, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border })

        local ColumnsFrame = CreateObj("Frame", { Parent = ContentArea, BackgroundTransparency = 1, BorderSizePixel = 0, Position = UDim2.new(0,8,0,8), Size = UDim2.new(1,-16,1,-16), Visible = false })
        local LeftColumn = CreateObj("ScrollingFrame", { Parent = ColumnsFrame, BackgroundTransparency = 1, BorderSizePixel = 0, Position = UDim2.new(0,0,0,0), Size = UDim2.new(0.5,-3,1,0), ScrollBarThickness = 2, ScrollBarImageColor3 = Library.Configuration.Accent, CanvasSize = UDim2.new(0,0,0,0), AutomaticCanvasSize = Enum.AutomaticSize.Y })
        CreateObj("UIListLayout", { Parent = LeftColumn, FillDirection = Enum.FillDirection.Vertical, HorizontalAlignment = Enum.HorizontalAlignment.Left, VerticalAlignment = Enum.VerticalAlignment.Top, Padding = UDim.new(0,4), SortOrder = Enum.SortOrder.LayoutOrder })

        local RightColumn = CreateObj("ScrollingFrame", { Parent = ColumnsFrame, BackgroundTransparency = 1, BorderSizePixel = 0, Position = UDim2.new(0.5,3,0,0), Size = UDim2.new(0.5,-3,1,0), ScrollBarThickness = 2, ScrollBarImageColor3 = Library.Configuration.Accent, CanvasSize = UDim2.new(0,0,0,0), AutomaticCanvasSize = Enum.AutomaticSize.Y })
        CreateObj("UIListLayout", { Parent = RightColumn, FillDirection = Enum.FillDirection.Vertical, HorizontalAlignment = Enum.HorizontalAlignment.Left, VerticalAlignment = Enum.VerticalAlignment.Top, Padding = UDim.new(0,4), SortOrder = Enum.SortOrder.LayoutOrder })
        ...
        -- Unique ID counter per tab, so IDs don't collide across boxes
        ...
        local function BuildBox(parent, counter, label)
            ...
            local WrapperFrame = CreateObj("Frame", { Parent = parent, BackgroundColor3 = Color3.fromRGB(10,10,10), BorderSizePixel = 0, Size = UDim2.new(1,0,0,0), AutomaticSize = Enum.AutomaticSize.Y, LayoutOrder = counter, ClipsDescendants = false })
            CreateObj("UIStroke", { Parent = WrapperFrame, Color = Library.Configuration.Accent, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border })

            local BoxFrame = CreateObj("Frame", { Parent = WrapperFrame, BackgroundColor3 = Color3.fromRGB(10,10,10), BorderSizePixel = 0, Position = UDim2.new(0,1,0,1), Size = UDim2.new(1,-2,0,0), AutomaticSize = Enum.AutomaticSize.Y, ClipsDescendants = false })
            CreateObj("UIPadding", { Parent = BoxFrame, PaddingLeft = UDim.new(0,6), PaddingRight = UDim.new(0,6), PaddingTop = UDim.new(0,4), PaddingBottom = UDim.new(0,6) })
            CreateObj("UIListLayout", { Parent = BoxFrame, FillDirection = Enum.FillDirection.Vertical, HorizontalAlignment = Enum.HorizontalAlignment.Left, VerticalAlignment = Enum.VerticalAlignment.Top, Padding = UDim.new(0,4), SortOrder = Enum.SortOrder.LayoutOrder })

            if label and label ~= "" then
                CreateObj("TextLabel", { Parent = BoxFrame, BackgroundTransparency = 1, Size = UDim2.new(1,0,0,14), Text = label, TextColor3 = Color3.fromRGB(200,200,200), TextSize = 12, FontFace = UIFont, TextXAlignment = Enum.TextXAlignment.Left, TextYAlignment = Enum.TextYAlignment.Center, RichText = false, LayoutOrder = 0 })
            end

            local ContentList = CreateObj("Frame", { Parent = BoxFrame, BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(1,0,0,0), AutomaticSize = Enum.AutomaticSize.Y, LayoutOrder = 1 })
            CreateObj("UIListLayout", { Parent = ContentList, FillDirection = Enum.FillDirection.Vertical, HorizontalAlignment = Enum.HorizontalAlignment.Left, VerticalAlignment = Enum.VerticalAlignment.Top, Padding = UDim.new(0,5), SortOrder = Enum.SortOrder.LayoutOrder })

            local Box = {}
            Box.Content = ContentList

            function Box:AddTitle(text)
                local Row = CreateObj("Frame", { Parent = self.Content, BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(1,0,0,16), LayoutOrder = #self.Content:GetChildren(), ClipsDescendants = false })
                local TitleLabel = CreateObj("TextLabel", { Parent = Row, BackgroundTransparency = 1, Position = UDim2.new(0,0,0,0), Size = UDim2.new(1,0,1,0), Text = text or "", TextColor3 = Color3.fromRGB(200,200,200), TextSize = 12, FontFace = UIFont, TextXAlignment = Enum.TextXAlignment.Left, TextYAlignment = Enum.TextYAlignment.Center, RichText = false })
                local rightOffset = 0
                local function shrinkTitle(px) rightOffset = rightOffset + px; TitleLabel.Size = UDim2.new(1,-rightOffset,1,0) end
                ...
                function TitleObj:AddKeyPicker(KParams)
                    ...
                    shrinkTitle(36)
                    local KBox = CreateObj("Frame", { Parent = Row, BackgroundColor3 = Color3.fromRGB(10,10,10), BorderSizePixel = 0, AnchorPoint = Vector2.new(1,0.5), Position = UDim2.new(1,0,0.5,0), Size = UDim2.new(0,32,0,12), ZIndex = 6 })
                    ...
                    trackConn(UserInputService.InputBegan:Connect(function(input, processed)
                        ...
                        if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == CurrentKey then KCallback(input.KeyCode) end
                    end))
                    local KP = {}
                ...
                function TitleObj:AddColorPicker(CParams)
                    ...
                    local PW, PH = 160, 160; local HH = 12; local POPUP_H = PH + HH + 12 + 6
                    local function buildPopup()
                        ...
                        local function applyColor()
                            local r,g,b = hsvToRgb(ch,cs,cv); CurrentColor = Color3.new(r,g,b)
                            colorSwatch.BackgroundColor3 = CurrentColor; svArea.BackgroundColor3 = Color3.fromHSV(ch,1,1)
                            svCursor.Position = UDim2.new(cs,0,1-cv,0); hueCursor.Position = UDim2.new(ch,0,0.5,0); CCallback(CurrentColor)
                        end
                        ...
                        local function updateSV(inputX, inputY) local abs = svArea.AbsolutePosition; local size = svArea.AbsoluteSize; cs = math.clamp((inputX-abs.X)/size.X,0,1); cv = math.clamp(1-(inputY-abs.Y)/size.Y,0,1); applyColor() end
                        ...
                        local function updateHue(inputX) local abs = hueBar.AbsolutePosition; local size = hueBar.AbsoluteSize; ch = math.clamp((inputX-abs.X)/size.X,0,1); applyColor() end
                        ...
                        UserInputService.InputChanged:Connect(function(input) if input.UserInputType ~= Enum.UserInputType.MouseMovement then return end; if svDragging then updateSV(input.Position.X,input.Position.Y) end; if hueDragging then updateHue(input.Position.X) end end)
                        local function repositionPopup() local abs = colorSwatch.AbsolutePosition; local pw = popupFrame.AbsoluteSize.X; local sx = ScreenGui.AbsoluteSize.X; local px = math.clamp(abs.X-pw/2,4,sx-pw-4); popupFrame.Position = UDim2.new(0,px,0,abs.Y+16) end
                        task.defer(repositionPopup)
                        local closeConn
                        closeConn = UserInputService.InputBegan:Connect(function(input)
                            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                                task.defer(function()
                                    local m = game.Players.LocalPlayer:GetMouse(); local abs = popupFrame and popupFrame.AbsolutePosition; local size = popupFrame and popupFrame.AbsoluteSize
                    ...
                    SwatchBtn.MouseButton1Click:Connect(function() if popupOpen then popupOpen = false; if popupFrame then popupFrame:Destroy(); popupFrame = nil end else popupOpen = true; buildPopup() end end)
                    local CP2 = {}
            ...
            function Box:AddButton(Params)
                ...
                local Btn = CreateObj("TextButton", { Parent = Row, BackgroundColor3 = Color3.fromRGB(10,10,10), BorderSizePixel = 0, Size = UDim2.new(1,0,1,0), Text = Title, TextColor3 = Color3.fromRGB(200,200,200), TextSize = 12, FontFace = UIFont, TextXAlignment = Enum.TextXAlignment.Center, TextYAlignment = Enum.TextYAlignment.Center, AutoButtonColor = false, ZIndex = 5 })
            ...
            function Box:AddToggle(Params)
                ...
                local rightOffset = 0
                local function shrinkTitle(px) rightOffset = rightOffset + px; TitleLabel.Size = UDim2.new(1,-(18+rightOffset),1,0); HitBtn.Size = UDim2.new(1,-rightOffset,1,0) end

                local Toggle = {}

                -- Internal setter (no callback duplicate on load)
                local function applyState(value, fireCallback)
                    State = value; Fill.Visible = State
                    if fireCallback then Callback(State) end
                end

                HitBtn.MouseButton1Click:Connect(function() applyState(not State, true) end)
                ...
                function Toggle:AddKeyPicker(KParams)
                    ...
                    trackConn(UserInputService.InputBegan:Connect(function(input, processed)
                        if processed or Listening then return end
                        if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == CurrentKey then KCallback(not State) end
                    end))
                    local KP = {}
                    function KP:Set(key) CurrentKey = key; KLabel.Text = key.Name end
                    function KP:Get() return CurrentKey end
                ...
                function Toggle:AddColorPicker(CParams)
                    ...
                    CreateObj("UIStroke", { Parent = colorSwatch, Color = Library.Configuration.Accent, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border })
                    local SwatchBtn = CreateObj("TextButton", { Parent = colorSwatch, BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(1,0,1,0), Text = "", ZIndex = 8 })
                    local PW, PH = 160, 160; local HW, HH = 160, 12; local POPUP_H = PH + HH + 12 + 6
                    local function buildPopup()
                        popupFrame = CreateObj("Frame", { Parent = ScreenGui, BackgroundColor3 = Color3.fromRGB(20,20,20), BorderSizePixel = 0, Size = UDim2.new(0,PW+16,0,POPUP_H), Position = UDim2.new(0,0,0,0), ZIndex = 50, ClipsDescendants = false })
                        CreateObj("UIStroke", { Parent = popupFrame, Color = Library.Configuration.Accent, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border })
                        ...
                        CreateObj("UIStroke", { Parent = hueCursor, Color = Color3.fromRGB(0,0,0), Thickness = 1 })
                        local function applyColor()
                            local r,g,b = hsvToRgb(h,s,v); CurrentColor = Color3.new(r,g,b)
                            colorSwatch.BackgroundColor3 = CurrentColor; svArea.BackgroundColor3 = Color3.fromHSV(h,1,1)
                            svCursor.Position = UDim2.new(s,0,1-v,0); hueCursor.Position = UDim2.new(h,0,0.5,0); CCallback(CurrentColor)
                        end
                        local svDragging = false; local hueDragging = false
                        local svHit = CreateObj("TextButton", { Parent = svArea, BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(1,0,1,0), Text = "", ZIndex = 56 })
                        local function updateSV(inputX, inputY) local abs = svArea.AbsolutePosition; local size = svArea.AbsoluteSize; s = math.clamp((inputX-abs.X)/size.X,0,1); v = math.clamp(1-(inputY-abs.Y)/size.Y,0,1); applyColor() end
                        svHit.MouseButton1Down:Connect(function() svDragging = true; local m = game.Players.LocalPlayer:GetMouse(); updateSV(m.X,m.Y) end)
                        local hueHit = CreateObj("TextButton", { Parent = hueBar, BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(1,0,1,0), Text = "", ZIndex = 56 })
                        local function updateHue(inputX) local abs = hueBar.AbsolutePosition; local size = hueBar.AbsoluteSize; h = math.clamp((inputX-abs.X)/size.X,0,1); applyColor() end
                        hueHit.MouseButton1Down:Connect(function() hueDragging = true; local m = game.Players.LocalPlayer:GetMouse(); updateHue(m.X) end)
                        UserInputService.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then svDragging = false; hueDragging = false end end)
                        UserInputService.InputChanged:Connect(function(input) if input.UserInputType ~= Enum.UserInputType.MouseMovement then return end; if svDragging then updateSV(input.Position.X,input.Position.Y) end; if hueDragging then updateHue(input.Position.X) end end)
                        local function repositionPopup() local abs = colorSwatch.AbsolutePosition; local pw = popupFrame.AbsoluteSize.X; local sx = ScreenGui.AbsoluteSize.X; local px = math.clamp(abs.X-pw/2,4,sx-pw-4); popupFrame.Position = UDim2.new(0,px,0,abs.Y+16) end
                    ...
                    -- Register color picker for config too
            ...
            function Box:AddSlider(Params)
                ...
                local function updateFromX(inputX)
                    ...
                    if newVal ~= Value then Value = newVal; SliderFill.Size = UDim2.new(pctFor(Value),0,1,0); ValueLabel.Text = tostring(Value).."/"..tostring(Max); Callback(Value) end
                end
                ...
                function Slider:Set(value)
                    ...
                    value = math.clamp(value, Min, Max); Value = value
            ...
            function Box:AddTextBox(Params)
                ...
                local Row = CreateObj("Frame", { Parent = self.Content, BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(1,0,0,0), AutomaticSize = Enum.AutomaticSize.Y, LayoutOrder = #self.Content:GetChildren(), ClipsDescendants = false })
                ...
                local InputFrame = CreateObj("Frame", { Parent = Row, BackgroundColor3 = Color3.fromRGB(10,10,10), BorderSizePixel = 0, Position = UDim2.new(0,0,0,18), Size = UDim2.new(1,0,0,16) })
                CreateObj("UIStroke", { Parent = InputFrame, Color = Library.Configuration.Accent, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border })
                local InputBox = CreateObj("TextBox", { Parent = InputFrame, BackgroundTransparency = 1, BorderSizePixel = 0, Position = UDim2.new(0,6,0,0), Size = UDim2.new(1,-12,1,0), Text = Default, PlaceholderText = InputText, PlaceholderColor3 = Color3.fromRGB(80,80,80), TextColor3 = Color3.fromRGB(200,200,200), TextSize = 12, FontFace = UIFont, TextXAlignment = Enum.TextXAlignment.Left, TextYAlignment = Enum.TextYAlignment.Center, ClearTextOnFocus = false, ClipsDescendants = true, ZIndex = 5 })
                InputBox.FocusLost:Connect(function() Callback(InputBox.Text) end)

                local TextBox = {}
                function TextBox:Set(value) InputBox.Text = tostring(value); Callback(InputBox.Text) end
                function TextBox:Get() return InputBox.Text end
                function TextBox:SetPlaceholder(text) InputBox.PlaceholderText = tostring(text) end

                Library:RegisterConfigItem(cfgId,
                    function() return InputBox.Text end,
                    function(val) InputBox.Text = tostring(val or ""); Callback(InputBox.Text) end
                )
            ...
            function Box:AddDropdown(Params)
                ...
                local Open = false; local optionButtons = {}; local selectedSet = {}; local CurrentValue
                ...
                local function buildHeaderText()
                    ...
                    local parts = {}; for _, v in ipairs(Values) do if selectedSet[v] then parts[#parts+1] = v end end
                ...
                local function toggleMulti(val)
                    ...
                    local out = {}; for _, v in ipairs(Values) do if selectedSet[v] then out[#out+1] = v end end; Callback(out)
                end
                ...
                function Dropdown:Set(value)
                    if Multi then
                        ...
                        selectedSet = {}; for _, v in ipairs(value) do selectedSet[v] = true end
                ...
                function Dropdown:Refresh(newValues)
                    ...
                    if not Multi then
                        local found = false; for _, v in ipairs(Values) do if v == CurrentValue then found = true; break end end
                        if not found then CurrentValue = Values[1] end; ValueLabel.Text = buildHeaderText()
                    else
                        local validSet = {}; for _, v in ipairs(Values) do validSet[v] = true end
                ...
                -- Config: single = string, multi = array of strings
                Library:RegisterConfigItem(cfgId,
                    function()
                        if Multi then
                            local out = {}; for _, v in ipairs(Values) do if selectedSet[v] then out[#out+1] = v end end; return out
        ...
        local Tab = {}
        function Tab:AddLeftBox(label)  local box; box, boxCountLeft  = BuildBox(LeftColumn,  boxCountLeft,  label); return box end
        function Tab:AddRightBox(label) local box; box, boxCountRight = BuildBox(RightColumn, boxCountRight, label); return box end
        return Tab
    end
    ...
    function Window:BuildUITab(tabName, isThemeCustomizable)
        ...
        MenuToggle:AddKeyPicker({
            ...
            Function = function(val) MenuToggle:Set(val) end
        })
        ...
        local ConfigDropdown = ConfigBox:AddDropdown({
            ...
            Function = function(val) selectedConfig = val end,
        })
        ...
        ConfigBox:AddButton({
            ...
            Function = function()
                ...
                if name == "" then Library:Notify("{yellow}Config name cannot be empty.{/yellow}", 3); return end
                local data = Library:CollectConfig()
                if saveConfig(name, data) then
                    local fresh = listConfigs(); ConfigDropdown:Refresh(fresh)
                    selectedConfig = name; ConfigDropdown:Set(name)
        ...
        ConfigBox:AddButton({
            Title    = "Load Config",
            Function = function()
                if not selectedConfig or selectedConfig == "" then Library:Notify("{yellow}No config selected.{/yellow}", 3); return end
        ...
        ConfigBox:AddButton({
            ...
            Function = function()
                ...
                if deleteConfig(name) then
                    local fresh = listConfigs(); selectedConfig = fresh[1] or nil
                    ConfigDropdown:Refresh(fresh)
                    if selectedConfig then ConfigDropdown:Set(selectedConfig) end
